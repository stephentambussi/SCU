...
int dfs(int idx)
{
    ...
    if (process[idx].mark)
    {
        ...
        process[idx].pid = -process[idx].pid;
        in_deadlock_chain = 1;
        return 1;
    }
    process[idx].mark = 1;
    ...
    while (current != NULL)
    {
        printf("%d waiting for resource:%s held by %d\n", process[idx].pid, current->resource, resource2pid[current->resource]);
        if (dfs(pid2idx[resource2pid[current->resource]]))
        {
            if (in_deadlock_chain)
                printf("Deadlock pid:%d\n", abs(process[idx].pid));
            if (process[idx].pid < 0)
                in_deadlock_chain = 0;
            return 1;
        }
        current = current->next;
...
int main(void)
{
    ...
    for (i = 0; i < n; i++)
    {
        printf("%d:pid:%d has lock on: ", i, process[i].pid);
