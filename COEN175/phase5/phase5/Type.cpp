...
int Type::size() const
{
    if(isInteger())
    {
        if(_specifier == "int") return 4; //integer
        else return 1; //char
    }
    else if(isCallback() || (_kind == SCALAR && _indirection > 0)) return 4; //callback or pointers
    else if(isArray()) //arrays
    {
        if(_indirection > 0) return _length * 4; //array of pointers
        else if(_specifier == "int") return _length * 4; //array of ints
        else if(_specifier == "char") return _length * 1; //array of chars
    }
    return 0; //temp error
}
