...
print(train_data_plus_bias) #X_train
...
print(test_data_plus_bias) #X_test
...
XT = tf.transpose(X)
w = tf.matmul(tf.matmul(tf.matrix_inverse(tf.matmul(XT, X)), XT), t) #w = inv(XT*X)*XT*t
